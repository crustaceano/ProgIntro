...
//        }
        ...
        long start_bytes = target >>> 32;
        for (long i = 0; i < (1L << 32); i++) {
            if (((start_bytes << 32) + start_bytes ^ i) == target) {
                print(4, (start_bytes << 32) + i, password);
            }
        }
    }
    ...
    static long Sum_1_n(long x) {
        if (x <= 0) {
            return 0L;
        }
        x %= PRIME;
        if (x % 2 == 0) {
            return x / 2 * (x + 1) % PRIME;
        } else {
            return (x + 1) / 2 * x % PRIME;
        }
    }
    ...
    private static void flag5(final byte[] password) {
        ...
        long result = 0L;
        long[] dells = {3, 5, 7, 2024};
        for (long del: dells) {
            result += (del * Sum_1_n(n / del - 1) + ((n % del) % PRIME) * ((n / del) % PRIME));
            result %= PRIME;
        }
